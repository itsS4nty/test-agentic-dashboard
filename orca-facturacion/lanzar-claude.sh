...
preparar() { # <fichero> <yo> <siguiente> <destino>
  # Sin la orquestación de Orca, los agentes se avisan a mano: se lo explicamos aquí.
  { cat "$1"
    echo
    echo "## Cómo avisas de que has terminado"
    echo
    echo '```'
    echo "orca orchestration send --from $2 --to $3 --run $RUN --subject 'Trabajo listo' --body '<qué has dejado y dónde>'"
    echo "orca orchestration send --from $2 --to run:$RUN --run $RUN --subject '$4' --body '<tu resumen de tres frases>'"
    echo '```'
    echo
